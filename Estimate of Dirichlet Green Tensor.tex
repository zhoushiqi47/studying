...
\begin{lem} \label{asym_frac}
	Let $F(\lambda,a)=\int_{0}^{a} t^{\alpha-1}f(t)e^{-\i\rho t}dt$ where $0<a\leq+\infty$, $0<\alpha<1$, $\rho>0$ and $t^{\alpha-1}f\in L^1(0,a)$, then we have
	\be
...
\begin{lem} \label{asym_frac_2}
	Let $F(\lambda,a)=\int_{0}^{a} t^{-1/2}f(t)e^{-\i\rho t}dt$ where $0<a\leq+\infty$ and $\rho>0$, then we have
	\be
...
Since $|t_2(\phi)|<\phi_\kappa$ or $|t_2(\phi)|<\pi/2-\phi_\kappa$ , we now define $\delta:=\min(\frac{\phi_\kappa}{2},\frac{\pi/2-\phi_\kappa}{2})$ and it is easy to see that
 ...
 Subtitating $t(s)=2\arcsin s/2$ for t in $I_{1}$, we can obtain
 \be\hspace{-1cm}
 I_{1}&=&\int_{s(-\delta)}^{s(\delta)}  f(t(s))\chi_\delta(t(s))\frac{1}{\sqrt{1-s^2/4}}e^{\i  \rho}e^{-\i \rho s^2/2} ds \\ \hspace{-1cm}
 &=&\int_{0}^{s(\delta)}  (f(t(s))\chi_\delta(t(s))+f(-t(s))\chi_\delta(-t(s)))\frac{1}{\sqrt{1-s^2/4}}e^{\i  \rho}e^{-\i \rho s^2/2} ds  \\ \hspace{-1cm}
 &:=& I_{11}+I_{12}
 \ee
 Taking substitution $s=\sqrt{x}$, we have
 \ben
 I_{11}&=&\frac{1}{2}\int_{0}^{\sqrt{2\sin \frac{\delta}{2}}}  f(t(\sqrt{x}))\chi_\delta(t(\sqrt{x}))\frac{1}{\sqrt{1-x/4}}x^{-1/2}e^{\i  \rho}e^{-\i \rho x/2} ds 
 \een
 
 
Observe that there exists $0<\delta<\min(\epsilon/2,\phi_\kappa,\pi/2-\phi_\kappa)$ that $\kappa/2<\sin(t+\phi)<(1+\kappa)/2$ for any $t\in(-\delta,\delta)$ and $\phi\in(\phi_\kappa-\delta,\phi_\kappa+\delta)$.
\ben
(\kappa+\sin(t+\phi))>3\kappa/2 \\
\cos(\frac{t+\phi+\phi_\kappa}{2})=\sqrt{\sin^2(\frac{t+\phi+\phi_\kappa}{2})}>\sqrt{1-((1+\kappa)/2)^2} \\
(\kappa^2-\sin^2(t+\phi))^{1/2} \\
=((\kappa+\sin(t+\phi))(\kappa-\sin(t+\phi))^{1/2}\\
=(\kappa+\sin(t+\phi))^{1/2}(\cos(\frac{t+\phi+\phi_\kappa}{2}))^{1/2}
(-2\sin(\frac{t+\phi-\phi_\kappa}{2}))^{1/2} \\
\frac{\pa f(t,\phi)}{\pa t}=\frac{\pa g_1(t,\phi)}{\pa t}+\frac{\pa g_2(t,\phi)}{\pa t}(\kappa^2-\sin^2(t+\phi))^{1/2}\\
+g_2(t,\phi)\frac{-2\cos(t+\phi)}{(\kappa^2-\sin^2(t+\phi))^{1/2}}\\
:=\frac{h(t,\phi)}{(\kappa^2-\sin^2(t+\phi))^{1/2}} \\
h(t,\phi)<M
\een
For any $\phi\in[0,\pi/2]$, we have
\ben
\int_{-\pi/2}^{\pi/2} \frac{\pa f(t,\phi)}{\pa t}
\een
\finproof
