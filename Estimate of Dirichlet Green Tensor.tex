...
\begin{lem}
	...
	\een
	where $N_*=\min\{N|\kappa^{2N-1}<c_0,N\geq2\}$.
\end{lem}
\debproof
...
\ee
where $p_n=\sum_{0}^{n-2}a^n_{k}t^k$ is a $(n-2)$-th polynomial such that its  coefficients have following recursion formula:
\ben
...
\be
\psi^{(2N_*)}(t,\tau)\geq\psi^{(2N_*)}(0,\tau)\geq\psi^{(2N_*)}(0,c_0)\geq\frac{c_0}{\kappa^{2N_*-1}}-1>0
\ee
