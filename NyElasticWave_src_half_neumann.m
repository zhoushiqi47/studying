function [Data,M,H] = NyElasticWave_src_half_neumann(omegas, lambda, mu, n, bctype, n_src, n_recv, source, receiver)
...
M=zeros(2*n,n,3);
ntt=5;
tt=linspace(-0.999,0.999,ntt);
tt((nnt+1)/2)=tt((nnt+1)/2)+0.0001;
aa=[t.*(t-1)/2; 1-t.*t ; t.*(1+t)/2];
%% compute the cauchy principal integral
for j=0:2*n-1
    x1j=repmat(x1(j),1,ntt);
    x2j=repmat(x2(j),1,ntt);
    dx1j=repmat(x1(j),2,ntt);
    dx2j=repmat(x2(j),2,ntt);
    distancej = sqrt( dxj1.*dxj1+dxj2.*dxj2 );
    for i=0:n-1
        tti=(2*i+1+t)/n;
        [x1i,x2i]=circlebc(tti,1); 
        [dxi1,dxi2]=circlebc(tti,2); 
        distancei = sqrt( dxi1.*dxi1+dxi2.*dxi2 );
        h = ((x1i-x1j).*dx1j+(x2j-x2i).*dx2j).*distancej./((x1i-x1j).^2+(x2i-x2j).^2)./distancei;
        if j==2*i
            h=h.*(tt+0.999);
            M(j,i,1)=pi/n*(h-h(1))./(tt+0.999).*aa(1,:);
            M(j,i,2)=0;
            M(j,i,3)=0;
        else if j==mod(2*i+2,2*n)
                h=h.*(tt-0.999);
                M(j,i,1)=0;
                M(j,i,2)=0;
                M(j,i,3)=pi/n*(h-h(ntt))./(tt-0.999).*aa(3,:);
            else if j==(2*i+1)
                    h=h.*(tt-0.0001);
                    M(j,i,1)=0;
                    M(j,i,1)=pi/n*(h-h((ntt+1)/2))./(tt-0.0001).*aa(1,:);
                    M(j,i,3)=0;
                else
 
                    for l=1:3
                        M(j,i,l)=pi/n*sum(h.*aa(l,:));
                    end
                end
            end
        end
...
for kk=1:Nomegas
            ...
            if (j==k)
                ...
                for l=1:2
                    for m=1:2
                        ...
                        H{l,m}(j,k) = dist*(H{l,m}(j,k)+G(l,m));
                    end
                end
            else
                dist = distance(k);
    ...
    f(2*n+1:end,:) = [reshape(Gs(:,3), 2*n ,n_src),reshape(Gs(:,4), 2*n ,n_src)];
    toc
    disp('右端项计算完成');
