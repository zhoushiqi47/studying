...
for ix=  1:Nx
    for iz=1:Nz
        for k=1:N
            ...
            if n_src==n_recv
               Gr=TractionDGreenTensor_2D2(omega(k),kp(k),ks(k),[x(ix)*ones(1,n_recv); z(iz)*ones(1,n_recv)],receiver);
               Gs=Gr;
            else
               Gr=TractionDGreenTensor_2D2(omega(k),kp(k),ks(k),[x(ix)*ones(1,n_recv); z(iz)*ones(1,n_recv)],receiver);
               Gs=TractionDGreenTensor_2D2(omega(k),kp(k),ks(k),[x(ix)*ones(1,n_src); z(iz)*ones(1,n_src)],source);
            end
